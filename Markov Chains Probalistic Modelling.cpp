...
{	char filename[64];
	int timestep;		
	printf_s("please type in the parameters in following order:\nperiod(um) K1D(cm-1) K2D(cm-1) Krad(cm-1) Ki(cm-1) reflectivity(0-1) timestep\n");
	scanf_s("%lf%lf%lf%lf%lf%lf%d", &Period, &K1D, &K2D, &Krad, &Ki,& refl,&timestep); //0.287 1334 386 38 5 0 4000 
	printf_s("please type in saving name(includes '.csv')\n");	
	scanf_s("%s", filename, sizeof(filename)); //Name.csv
	printf_s("the provided parameters are: \nperiod = %lf mm\nK1D = %lf cm-1\nK2D = %lf cm-1\nKrad = %lf cm-1\nKi = %lf cm-1\nreflecvtivity = %lf\ntarget timestep = %d\n", Period, K1D, K2D, Krad, Ki, refl, timestep);
	printf_s("%s\n", filename);
	Initialize();
	int OrderArray[] = {700};//100,200,300,400,500,600,700,800,900,1000 change the array to meet the requirement of size, multiple size in one simulation is achievable, but the orderarray should match the i below 	
